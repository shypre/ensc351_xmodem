...
cs1stBlk()
{
    ...
    {
        checksum += blkBufs[next][i];
    }
    blkBufs[next][3+CHUNK_SZ] = checksum;
}
...
void SenderX::sendBlkPrepNext()
{
	...
	uint8_t lastByte = sendMostBlk(blkBufs[next]);
	sendLastByte(lastByte);

    //move next blkBuf into curr
    memcpy(blkBufs[curr], blkBufs[next], BLK_SZ_CRC);

    genBlk(blkBufs[next]); // prepare next block
}
...
void SenderX::resendBlk()
{
    ...
    uint8_t lastByte = sendMostBlk(blkBufs[curr]);
    sendLastByte(lastByte);
}
